...
void showLidarTopview()
{
    ...
    for (auto it = lidarPoints.begin(); it != lidarPoints.end(); ++it)
    {
        ...
        float zw = (*it).z; // world position in m with z facing up from the sensor

        int y = (-xw * imageSize.height / worldSize.height) + imageSize.height;
        int x = (-yw * imageSize.width / worldSize.width) + imageSize.width / 2;

        // uncomment to view raw image
        ...
        if(zw > -1){
            float height = it->x;
            float global_height = worldSize.height;
            int red = min(255, (int)(255 * abs((height - global_height) / global_height)));
            int green = min(255, (int)(255 * (1 - abs((height - global_height) / global_height))));
